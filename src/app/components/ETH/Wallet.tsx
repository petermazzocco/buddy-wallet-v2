  ...
  useEffect(() => {
    async function getNftsFromBuddy() {
      try {
        ...
        const nftsIterable = ethAlchemy.nft.getNftsForOwnerIterator(
          buddy as Address
        );
  ...
  useEffect(() => {
    async function getTokensFromBuddy() {
      try {
        ...
        const allTokens = await ethAlchemy.core.getTokensForOwner(
          buddy as Address
        );
    ...
    async function getTransactionsFromBuddy() {
      try {
        let allTransactions: AssetTransfersResponse =
          await ethAlchemy.core.getAssetTransfers({
            fromBlock: "0x0",
